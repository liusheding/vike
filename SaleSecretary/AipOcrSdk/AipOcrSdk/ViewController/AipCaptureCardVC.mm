...
- (IBAction)pressCheckChoose:(id)sender {
    ...
    switch (self.cardType) {
        case CardTypeIdCardFont:{
            [[AipOcrService shardService] detectIdCardFrontFromImage:self.finalImage withOptions:nil successHandler:^(id result) {
        ...
        case CardTypeIdCardBack:
        {
            ...
            [[AipOcrService shardService] detectIdCardBackFromImage:self.finalImage withOptions:nil successHandler:^(id result) {
                if ([self.delegate respondsToSelector:@selector(ocrOnIdCardSuccessful:)]) {
                    [self.delegate ocrOnIdCardSuccessful:result];
                }
            } failHandler:^(NSError *err) {
        ...
        case CardTypeBankCard: {
            ...
            [[AipOcrService shardService] detectBankCardFromImage:self.finalImage successHandler:^(id result) {
...
- (void)detectLocalIdcard:(BOOL)isBack{
    ...
    switch (model.image_status) {
        ...
        case idcard_quality::IDCARD_WRONG_LOCATION:{
            ...
            tips = [NSString stringWithFormat:@"请将身份证正对于取景框内"];
            dispatch_semaphore_signal(_sema);
            break;
        }
        ...
        case idcard_quality::IDCARD_NORMAL:{
            ...
            if (isBack) {
                ...
                [[AipOcrService shardService] detectIdCardBackFromImage:self.finalImage withOptions:nil successHandler:^(id result) {
                    ...
                    wself.recognizedSucceed = YES;
                    [wself ocrOnIdCardSuccessful:result];
                } failHandler:^(NSError *err) {
...
- (void)alertView:(UIAlertView *)alertView didDismissWithButtonIndex:(NSInteger)buttonIndex {
    ...
    if (alertView.tag == 255) {
        ...
        if (buttonIndex == 1) {
            ...
            //ytodo 测试：检查剪切后的图片
            UIImageWriteToSavedPhotosAlbum(self.finalImage, self, @selector(image:didFinishSavingWithError:contextInfo:), (__bridge void *)self);
        }else{
